...
This guide covers how to set up the development environment and build Sonarus from source across different platforms.
...
The raw binary (`src-tauri/target/release/sonarus`) cannot run standalone — it needs Tauri resource files (tray icons, sounds, VAD model) to be co-located at the expected path.

**Install from the deb bundle** (works on any Linux distro):

```bash
cd /tmp
ar x /path/to/Sonarus-v2/src-tauri/target/release/bundle/deb/Sonarus_*_amd64.deb data.tar.gz
tar xzf data.tar.gz
sudo cp usr/bin/sonarus /usr/bin/
sudo cp -r usr/lib/Sonarus /usr/lib/
sudo cp -r usr/share/icons/hicolor/* /usr/share/icons/hicolor/
sudo cp usr/share/applications/Sonarus.desktop /usr/share/applications/
```

After subsequent rebuilds, only the binary needs re-copying:

```bash
sudo cp src-tauri/target/release/sonarus /usr/bin/
```
...
```
Bundling Sonarus_*_amd64.AppImage
...
~/.cache/tauri/linuxdeploy-x86_64.AppImage --appimage-extract-and-run \
  --appdir Sonarus.AppDir --plugin gtk --output appimage
```
